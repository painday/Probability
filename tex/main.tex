...
\end{equation}
并且对于任意的 $1\leq k\leq m=2^{n-1}-1,$ 若 $k$ 的二进制表示为 $k=\sum_{j=0}^s k_j2^j=(k_s\cdots k_1k_0)_2,$ 其中 $1\leq s<n,\ k_j\in\{0,1\},\ k_s=1,$ 那么就有
...
而式 \eqref{endpoints of Cantor set} 就是说，$\bigcup_{k=1}^{n-1}G_k$ 中开区间的端点集 $\{a_k\}_{1\leq k\leq 2^{n'}-1}$ 恰好等于一切最后一位为 $1,$ 其余位均不为 $1$ 的不超过 $n'$ 位的 $3$ 进制正小数 $(0.m_1m_2\cdots m_{s-1}1)_3\ (0<s<n)$ 之全体所做成的集合. 类似地，端点集 $\{b_k\}_{1\leq k\leq 2^{n'}-1}$ 
...
\begin{equation}
    f_k=b_k+{2\over 3^n}
\end{equation}
因而 $e_k$ 遍历了一切最后一位为 $1,$ 其余位均不为 $1$ 的 $n$ 位 $3$ 进制正小数 $(0.m_1m_2\cdots m_{n-1}1)_3\ (0<s<n),$ 以及 $f_k$ 遍历了一切每一位都不为 $1$ 且最后一位为 $2$ 的 $n$ 位 $3$ 进制小数 $(0.m_1m_2\cdots m_{n-1}m_n)_3\ (0<s<n).$ 这表明结论 \eqref{structure of Cantor set} 和 \eqref{endpoints of Cantor set} 在 $n=u+1$ 时成立，从而完成了证明.
