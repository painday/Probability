...
\end{example}
事实上，假设 $\mathrm{Im}(\mu) = [0,1]\cup [3,c],$ 那么显然 $\mu$ 为有限测度. 设 $\mu(E)=1,\mu(F)=3,$ 那么就有 $3\leq\mu(E\cup F)\leq c.$ 
显然，若 $\mu(E\cap F)=\mu(E),$ 那么就有 $\mu(F\setminus E)=2,$ 这与 $\mathrm{Im}(\mu)= [0,1]\cup [3,c]$ 矛盾. 因此 $\mu(E\cup F)=\mu(E)+\mu(F)-\mu(E\cap F)>\mu(F)=3.$ 

将 $\mathcal S$ 划分为 $\mu(E)$ 落入 $[0,1]$ 的集类 $\mathcal A$ 和 $\mu(E)$ 落入 $[3,c]$ 的集类 $\mathcal B,$ 那么必定有 $\{A\cup A'|\ A,A'\in\mathcal A\}\subset\mathcal A,$ 以及 $\{B\setminus A|\ A\in\mathcal A,\ B\in\mathcal B\}\subset\mathcal B.$ 
记 $\mathcal C=\{B\setminus A|\ A\in\mathcal A,\ B\in\mathcal B\},$ 那么必定有 $\mathcal A$ 中集合必定与 $\mathcal C$ 中集合不交. 
