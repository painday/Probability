...
自然，记 $b_k$ 的 $3$ 进制小数位数为 $s=h(b_k),$ 那么就有 
\begin{equation}
    a_k = b_k-{s\over 3^{s}}=g(k)-{h\circ g(k )\over 3^s}
\end{equation}
...
${2b_k+a_{k+1}\over 3}=(0.\underbrace{0\cdots 0}_{n-s-1}\underbrace{[2k_s]\cdots [2k_2]0}_{s}21)_3={b_{k}\over 3}+{1\over 3^{n+1}}.$ 


否则，$k_1=0,$ 那么令 $q$ 为满足 $k_j=k_{j-1}=\cdots=k_0=0$ 的最大指标 $j,$ 则有 $j>1,$ 并且有 $k=(\underbrace{k_s\cdots k_{q+2} 1}_{s-q}\underbrace{0\cdots 0}_{q+1})_2,\ k+1=(\underbrace{k_s\cdots k_{q+2}1}_{s-q}\underbrace{0\cdots 01}_{q+1})_2.$
...
${2b_k+a_{k+1}\over 3}=(0.\underbrace{0\cdots 0}_{n-s-1}[1+k_s]\underbrace{[2k_{s-1}][2k_{s-2}]\cdots [2k_{q+2}][k_{q+1}+1]}_{s-q}\underbrace{0\cdots 01}_{q+1})_3,$ 也即
\begin{equation}
    e_k=(0.\underbrace{0\cdots 0}_{n-s-1}\underbrace{[2k_s][2k_{s-1}][2k_{s-2}]\cdots [2k_{q+2}][2k_{q+1}]}_{s-q+1}\underbrace{0\cdots 01}_{q+1})_3
\end{equation}
也就是说，对于任意的 $1\leq k\leq 2^{n}-1,$ 均有(在 $k$ 的二进制位全为 $1$ 时约定 $q=0$)
\begin{equation}
    e_k=(0.\underbrace{0\cdots 0}_{n-s-1}\underbrace{[2k_s][2k_{s-1}][2k_{s-2}]\cdots [2k_{q+2}][2k_{q+1}]}_{s-q+1}\underbrace{0\cdots 01}_{q+1})_3={a_{k+1}\over 3}
\end{equation}
成立. 自然，有
\begin{equation}
    f_k=(0.\underbrace{0\cdots 0}_{n-s-1}\underbrace{[2k_s][2k_{s-1}][2k_{s-2}]\cdots [2k_{q+2}][2k_{q+1}]}_{s-q+1}\underbrace{0\cdots 02}_{q+1})_3={b_{k+1}\over 3}
\end{equation}

一般地，
\begin{equation}
    a_{k+1}=a_k+{1\over 3^{n}}+{1\over 3^{n-f(k)}}=b_k+{1\over 3^n}
\end{equation}
因此
\begin{equation}
    \left\{
        \begin{aligned}
            e_k =& b_k+{1\over 3^{n+1}},\\
            f_k =& b_k+{2\over 3^{n+1}}
        \end{aligned}
    \right.
\end{equation}
因而 $e_k$ 遍历了一切最后一位为 $1,$ 其余位均不为 $1$ 的不超过 $n+1$ 位的 $3$ 进制正小数 $(0.m_1m_2\cdots m_{s-1}m_s1)_3\ (0<s<n).$ 这就证明了结论.
