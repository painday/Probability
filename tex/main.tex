...
\begin{equation}\label{Cantor set induction def}
    G_n=\bigcup_{k=0}^{m} \left({2c_k+d_k\over 3},{c_k+2d_k\over 3}\right)
\end{equation}
...
类似地，$G_2$ 正是那些所有可能的三进制表示的小数点后第一位不为 $1,$ 但第二位为 $1$ 的那些实数所组成的集合. 这是因为 $G_2$ 与 $G_1$ 不交，而 $G_1=(1/9,2/9)\cup (7/9,8/9)$ 分别是所有可能的三进制表示的小数点后第一位为零，第二位为一和第一位为二，第二位为一的那些 $[0,1]$ 中实数所做成的集合.
...
\begin{equation}
    \left\{\begin{aligned}
        ...
        a_4=&(0.\underbrace{0\cdots 100}_{n})_3,\ b_4=(0.\underbrace{0\cdots 110}_{n})_3\\
...
再接着的四项对应于只有最后三位不为零的情形. 可以看到，如果将只有最后 $k$ 项不等于零(倒数第 $k$ 位不等于零，并且最后一个不为零的位只能为 $1,$ 且 $1$ 只出现一次)的那些长度为 $n$ 的序列 $\{x_j\}$ 所做成的集合记为 $A_k,$ 那么就有递归公式
\begin{equation}
    A_{k+1}=\left\{{1\over 3^{n-k}}\right\}\cup \left[\bigcup_{1\leq j\leq k}\left(A_k+{1\over 3^{n-k}}\right)\right]
\end{equation}
...
\begin{equation}
    \left\{\begin{aligned}
        ...
        c_4=&(0.\underbrace{0\cdots 110}_{n})_3,\ d_4=(0.\underbrace{0\cdots 201}_{n})_3\\
