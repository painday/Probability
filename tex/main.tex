...
那么就有
\begin{equation}
    f^{-1}([a,+\infty))=\bigcap_n f_n^{-1}([a,+\infty))
\end{equation}
因而 $f^{-1}([a,+\infty))$ 是 $\mathcal S-$可测集.
另一方面，
\begin{equation}\label{eq:inverse_image_of_positive_infinity}
    f^{-1}(+\infty)=\bigcap_n \bigcup_{k} f_k^{-1}([n,+\infty))
\end{equation}
这是因为，若 $x\in \bigcap_n \bigcup_{k} f_k^{-1}([n,+\infty)),$ 那么对于每个 $n,$ 均存在某个 $k\in\mathbb N,$ 使得 $f_k(x)\geq n.$ 满足该条件的 $x$ 必定满足 $f(x)=+\infty.$ 另一方面，若 $f(x)=+\infty,$ 那么要么 $f_n(x)$ 对某个 $n$ 为 $+\infty,$ 要么 $\{f_n(x)\}_{n\in\mathbb N}$ 为上无界序列. 在这两种情况下，自然都有 $x\in \bigcap_n \bigcup_k f_k^{-1}([n,+\infty))$ 成立. 这就证明了等式 \eqref{eq:inverse_image_of_positive_infinity}. 类似地有
\begin{equation}\label{eq:inverse_image_of_negative_infinity}
    f^{-1}(-\infty)=\bigcap_n \bigcup_{k} f_k^{-1}((-\infty,-n])
\end{equation}
这就证明了 $f^{-1}(+\infty)$ 和 $f^{-1}(-\infty)$ 都是 $\mathcal S-$可测集，从而证明了 $f$ 的可测性. 类似可知 $\inf_n f_n$ 是可测的.
