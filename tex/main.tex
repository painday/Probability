...
\begin{equation}
    |\bigcup_{k=1}^n (r_k+V)|< n|V|=\sum_{k=1}^n |r_k+V|
\end{equation}
...
由于 $I_n$ 互不相交，因此对于任意固定的 $m,$ 容易验证 $\ell(J_k)\geq \sum_{n=1}^m \ell(J_k\cap I_n).$ 于是就有 $\ell(J_k)\geq\sum_n \ell(J_k\cap I_n),$ 从而有 
\begin{equation}
