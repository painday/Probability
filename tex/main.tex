...
这个证明花了我很长时间，尝试了很多办法. 看起来可选的思路很多，最后还是用了比较蛮力的方法.
...
事实上，记 $f$ 的不连续点集为 $D,$ 那么由例 \ref{ex:monotone_function_discontinuous_point_set_at_most_countable} 中结论可知 $D$ 是至多可数的，因而是 Borel 集合.
令 $\mathcal S=\{B\in\mathcal B|\ g(B) \text{是 Borel 可测集}\},$ 那么显然 $\mathcal S$ 对补集运算和可数并封闭. 要证明 $\mathcal S=\mathcal B,$ 只需证明每个有界非空开区间都落入 $\mathcal S$ 中即可. 
事实上，设 $a<b,$ 那么若 $g$ 在 $(a,b)$ 上连续，那么 $g((a,b))$ 就是一个区间，因而是 Borel 可测集. 若 $(a,b)\cap D\neq\emptyset,$ 那么
若 $\min_n a_n$ 存在，则可将 $a_n$ 枚举为 $a_1<a_2<\cdots,$ 于是由 
\begin{equation}
    g((a,b)) =  (f(a),f(a_1-))\cup \{f(a_1)\}\cup (f(a_1+),f(a_2-))\cup \{f(a_2)\}\cup\cdots 
\end{equation}
可知 $g((a,b))$ 为 Borel 可测集.  若 $\max_n a_n$ 存在，则可将 $(a,b)\cap D$ 枚举为 $a_1>a_2>\cdots,$ 于是有
\begin{equation}
    g((a,b)) =  (f(a_1+),f(b))\cup \{f(a_1)\}\cup (f(a_2+),f(a_1-))\cup \{f(a_2)\}\cup\cdots 
\end{equation}
可知 $g((a,b))$ 为 Borel 可测集.  若 $a_n$ 既没有最大元，也没有最小元，则可取某个 $x\in (a,b)\cap D,$ 并将 $a_n$ 枚举为 $x=a_1<a_2<\cdots,$ 与 $x=b_1>b_2>\cdots,$ 于是有
\begin{equation}
    \begin{aligned}
        g((a,b)) =& \left( (f(x),f(a_1-))\cup \{f(a_1)\}\cup (f(a_1+),f(a_2-))\cup \{f(a_2)\}\cup\cdots\right) \\
        &\cup \left((f(b_1+),f(x))\cup \{f(b_1)\}\cup (f(b_2+),f(b_1-))\cup \{f(b_2)\}\cup\cdots \right)
    \end{aligned}
\end{equation}
可知 $g((a,b))$ 为 Borel 可测集. 这就证明了 $\mathcal S=\mathcal B,$ 故 $f(B)=g(B)$ 为 Borel 集.

\begin{example}
    设 $B$ 为 Borel 集， $f:B\to\mathbb R$ 为单调递增函数，那么存在一列严格单调递增函数 $f_n:B\to\mathbb R,$ 使得 $f=\lim_n f_n.$
\end{example}
